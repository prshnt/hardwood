...
"""
Post-processes a Parquet file to tag a chosen BYTE_ARRAY column with the BSON
logical type in the Thrift footer. PyArrow does not write BSON annotations
natively (no `pa.bson_()` equivalent to `pa.json_()`), so this helper reads the
compact-Thrift footer, rewrites the target column's SchemaElement, and rewrites
the file. The rest of the file (data pages, row groups, statistics, etc.) is
preserved byte-for-byte.

Only the subset of parquet.thrift that PyArrow actually emits is modelled here;
if PyArrow starts writing additional fields that are not defined below, the
re-serialized footer will silently drop them, so keep this IDL in sync with
parquet-format when upgrading PyArrow.
"""
